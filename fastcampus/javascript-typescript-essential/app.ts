...
function updateView(html: string): void { //return 값이 없을때
  if(container) 
    container.innerHTML = html
  else
    console.error('최상위 컨테이너가 없어 UI를 삽입할 수 없습니다.')
}
...
function makeComment(comments: NewsComment[]): string {
  ...
  for(let i = 0; i < comments.length; i++) {
    const { level, user, time_ago, content, comments: commentList }: NewsComment = comments[i]
    ...
    if (comments.length > 0) {
      commentString.push(makeComment(commentList));
    }
  }
